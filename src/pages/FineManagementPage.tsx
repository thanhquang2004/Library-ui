...
const FineManagementPage: React.FC = () => {
  ...
  const fetchFines = useCallback(async () => {
    try {
      setLoading(true);
      setErrorMsg("");
      const { data } = await api.get<Fine[]>("/fines/search", {
        headers: { Authorization: `Bearer ${token}` },
      });

      console.log("✅ Fines fetched:", data);
      setFines(data);
    ...
    try {
      ...
      const overdueData: Overdue[] = data
        ...
        .map((l) => ({
          ...
          bookTitle: l.bookItem?.title || "Unknown",
          dueDate: l.dueDate,
          memberId: l.member,
        }));
    ...
    try {
      await api.post(
        "/fines",
        {
          ...
          bookLendingId: modalData.lendingId,
          amount,
          reason,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      ...
      logError("❌ Error creating fine", error);
              ...
              <tbody>
                {fines.map((fine) => (
                  <tr key={fine.fineId} className="border-t">
                    <td className="p-2">{fine.member.email}</td>
                    <td className="p-2">{fine.amount.toLocaleString()} VND</td>
        ...
        {showModal && modalData && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40">
            <div className="bg-white p-6 rounded shadow w-96">
              ...
              <p className="mb-2">
                <strong>Sách:</strong> {modalData.bookTitle}
              </p>
              <p className="mb-2">
